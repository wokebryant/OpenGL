...
public class PanoramaView extends ImageView implements View.OnTouchListener {
    ...
    public PanoramaView(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
        init();
    }

    public PanoramaView(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
    ...
    private void showCenter(){
        ...
        translationImage(new PointF(viewSize.x/2 - scaleSize.x/2,viewSize.y/2 - scaleSize.y/2));
        ...
        Log.i(TAG, " imageSize= " + imageSize.x + " " + imageSize.y );
        Log.i(TAG, " scaleSize= " + scaleSize.x + " " + scaleSize.y);
        Log.i(TAG, " bitmapOrigin= " + bitmapOriginPoint.x + " " + bitmapOriginPoint.y );
    ...
    private void moveRectToCenter() {
        ...
        float newLeft = bitmapRect.left - rectSize.left * doubleFingerScrole;
        float newRight = newLeft + (rectSize.right - rectSize.left) * doubleFingerScrole;
        float rectCenterX = (newLeft + newRight) / 2;
        float moveX;
        //判断矩形框中心在屏幕中心的左边还是右边
        if (rectCenterX < viewCenter.x) {
            if (viewCenter.x - rectCenterX > -bitmapOriginPoint.x) {
                moveX = -bitmapOriginPoint.x;
            } else {
                moveX = viewCenter.x - rectCenterX;
            }
        } else {
            if (rectCenterX - viewCenter.x > bitmapOriginPoint.x) {
                moveX = bitmapOriginPoint.x;
            } else {
                moveX = -(rectCenterX - viewCenter.x);
            }
        }

        translationImage(new PointF(moveX, 0f));
    }
    ...
    public boolean onTouch(View v, MotionEvent event) {
        switch (event.getAction() & MotionEvent.ACTION_MASK) {
            ...
            case MotionEvent.ACTION_MOVE:
                //手指移动时触发事件
                /**************************************移动*******************************************/
                if (/*zoomInMode != ZoomMode.Ordinary && */!isPointerUp) {
                    //如果是多指，计算中心点为假设的点击的点
                    float currentX = 0;
                    float currentY = 0;
                    //获取此时屏幕上被触碰的点有多少个
    ...
    public RectF getBitmapRect() {
        ...
        mapState.left = value[2];
        mapState.top = value[5];
        mapState.right = mapState.left + imageSize.x * value[0];
        mapState.bottom = mapState.top + imageSize.y * value[0];
        Log.i(TAG, " bitmapRect= " + mapState.left + " " + mapState.top + " " + mapState.right + " " + mapState.bottom );
        return mapState;
    }
